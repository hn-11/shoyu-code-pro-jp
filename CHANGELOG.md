...
- **基準を英語圏のターミナルフォントに置き換えた**（v5.0.0）。v4.0.0 までは
  ...
  - **Sumi Moji が持つ文字はすべて 1 セル**（ギリシャ・キリル・罫線・矢印
    ...
    ギリシャ・キリルが無いぶんこの規則が効き、ほとんどが直立と同じ 1 セルに
    なる（`Μ Ж М Ф Ш Щ Ъ Ы Ю ж ф ш щ ю` の 14 字は基準ウェイトで送り幅
    755〜1005、最重量でインク 672〜972 と広く描かれていて収まらず全角。
    `verify.py` がこの集合を固定して監視する）。幅は
...
- セルフレビューで見つけた不具合の修正:
  ...
  - **斜体面のギリシャ・キリルが 1 セルに揃っていなかった**。
    ...
    の直立面では正しく出るという状態だった。`narrow_letters` が
    セル / 送り幅の比率で 1 セルに詰める（半角ハングルと同じ計算なので
    サイドベアリングは比例して残る）。`verify.py` は例外の固定集合を
    やめ、「ギリシャ・キリルはすべて 1 セル」を検査する
