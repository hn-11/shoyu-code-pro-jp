...
例外がひとつ。バリアブル版の半角カナ28字は、Source Han Mono の静的面が
補間できないため、土台の Source Han Sans VF から取って Mono の収まりを
アフィンで移している。ここでは「上流の設計を正確に移す」ことと「軸全域で
正しく変化する」ことが両立せず、後者を優先した。
...
半角カナ28字だけは Source Han Mono ではなく**土台の Source Han Sans VF から
取り、Mono の収まりをアフィン変換で移している**。Mono は静的面しか無く、
Adobe がウェイトごとに重なり除去をしているため、この28字は面によって輪郭の
構造が違って補間できない。そのまま使うと Heavy で `ｷ` `ﾔ` が Regular の字形の
まま残る（ストローク太さが2.3倍ずれる）。土台は1つのバリアブルフォントなので
全域で正しく変化し、Mono の水平伸長（約1.32倍）も移せる。移せないのは Mono が
字ごとに入れた微調整だけで、差分面積で 3.7〜10.9%（数ユニットのエッジ位置）。
**静的版は補間が要らないので Mono をそのまま使う**ため、この28字は静的版と
VF 版でごく僅かに形が違う。
...
Nerd Fonts 版は**静的のみ**。これは未実装ではなく、やらないと決めた判断。

font-patcher（2296行）は VF を検出して "opening might fail" と警告するだけで
対応していない。自前で置き換えるには、コードポイント単位の伸縮規則60項目と
7種の伸縮モードを引き取ることになる — `xy-ratio` が 0.5 / 0.59 / 0.7 / 0.78 /
0.865、`overlap` が -0.03〜0.10 という、目視で詰めた較正値の集まりで、導出は
できない。それを Nerd Fonts のリリースごとに追従し、10412グリフ × 6ファイルの
見た目に責任を持つ割に、得られるのは「アイコン入りの可変フォント」だけで、
アイコンが要る人には静的42面が既にある。

font-patcher が VF を扱うようになったら再検討する。
