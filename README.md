...
移植するのは合字グリフと単独の `=` のみで、英数字や他の記号は SHCJ（= Source Code Pro）のまま。
`=` を Monaspace から取るのは、`=` と `==` `=>` `:=` でバーの間隔が違って見えないようにするため
（SCP は 170u、Monaspace は 219u）。幅と縦中心は SCP の `=` と 10u 以内で一致する。
`-` は Monaspace だと 132u 短く、`|` は 60u 上に寄るなど他の記号は骨格が合わないので持ち込まない。
