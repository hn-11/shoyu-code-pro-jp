...
Source Han Sans のギリシャ文字は 602〜795 と、セルを少し超える幅で
描かれているので、切り上げるとターミナル 1 桁分を余計に取ってしまう
（しかもウェイトによって 1 桁だったり 2 桁だったりする）。斜体は
Source Code Pro Italic にギリシャ・キリルが無いぶんこの規則が効き、
ほとんどが直立と同じ 1 セルになる。例外は `Μ Ж М Ф Ш Щ Ъ Ы Ю ж ф ш щ ю`
の 14 字で、Source Han Sans が基準ウェイトで送り幅 755〜1005、最重量で
インク 672〜972 と広く描いているため 1 セルには収まらず（あるいは全角の
ほうが近く）、斜体では全角（2 桁）になる。
...
| 行ボックスに対して描くその他（`` `` ブランチ・鍵・行番号など） | `^pa` | 縦横比を保ったまま、セル幅と行の全高のうち先に当たるほうに収める（`` は幅で決まって行の 45%、`` は行いっぱい） |
